...
int main(){
	...
	arvv_imprime(c);
	while(op > 0 || op < 6){
		printf("\n\n(1) - Inserir arquivo/diretorio  --  (2) - Destruir arquivo/diretorio  --  (3) - Buscar diretorio  --  (4) - Renomear diretorio  --  (5) - Mover diretorio  --  (6) - Transformar arquivo\nInsira o comando da operacao desejada: ");
		scanf("%d",&op);
		switch(op){
			case 0:
				exit(1);
			case 1:
				...
				if(aux == NULL){
					...
					if(valida == 'y' || valida == 'Y'){
						aloca_dir(entradatxt2,"c");
						//insere_irmao(c,inserido,entradatxt);
					}
				}
				...
				//insere_irmao(dir,inserido,entradatxt2);
		}
	}
	return 0;
}
...
Tree* aloca_dir(char* nome, char* dir){
	...
	novo->data_cria = insere_data(info->tm_mday,info->tm_mon,(info->tm_year + 1900),info->tm_hour,info->tm_min);
	novo->ultima_mod = insere_data(info->tm_mday,info->tm_mon,(info->tm_year + 1900),info->tm_hour,info->tm_min);
